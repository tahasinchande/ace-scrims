...
import { Trophy, Crosshair, Medal, Trash2 } from "lucide-react"
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-10 md:px-6">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_360px]">
          ...
          <section aria-label="Recent match results" className="flex flex-col gap-3">
            <h2 className="font-display text-lg font-bold uppercase tracking-wide">Recent Matches</h2>
                ...
                {recent.map((r) => (
                  <li key={r.id} className="glass flex flex-col gap-3 rounded-2xl p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <h3 className="min-w-0 flex-1 truncate font-display text-lg font-bold">{r.winningTeam}</h3>
                    ...
                    {/* Render multiple images if available (uses screenshotUrls JSONB) */}
                    {r.screenshotUrls && Array.isArray(r.screenshotUrls) && r.screenshotUrls.length > 0 ? (
                      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                        {r.screenshotUrls.map((u: string) => (
                          <Image
                            key={u}
                            src={u}
                    ...
                    {isModOrAdmin && (
                      <div className="mt-2 flex justify-end">
                        <form method="post" action={`/api/results/${r.id}/delete`}>
                          <button
                            type="submit"
                            className="inline-flex items-center gap-2 rounded-md bg-destructive px-3 py-1 text-sm font-semibold text-white"
                          >
          ...
          <aside aria-label="Team leaderboard" className="flex flex-col gap-3">
            <h2 className="font-display text-lg font-bold uppercase tracking-wide">Top Teams</h2>
            <div className="glass rounded-2xl p-2">
              {leaderboard.length === 0 ? (
                <p className="p-8 text-center text-sm text-muted-foreground">The leaderboard fills up as results are published.</p>
              ) : (
                <ol className="flex flex-col">
                  {leaderboard.map((team, i) => (
                    <li key={team.teamName} className="flex items-center gap-3 rounded-xl px-3 py-3 transition-colors hover:bg-secondary/50">
                      <span className={`flex size-8 shrink-0 items-center justify-center font-display text-sm font-bold ${i < 3 ? medalColor[i] : "text-muted-foreground"}`}>
                        {i < 3 ? <Medal className="size-5" /> : `#${i + 1}`}
                      </span>
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-display font-bold">{team.teamName}</p>
                        <p className="text-xs text-muted-foreground">{team.wins} {team.wins === 1 ? "win" : "wins"} · {team.totalKills} kills</p>
                      </div>
                      <span className="font-display text-sm font-bold text-primary">{formatTaka(team.totalPrizeBdt)}</span>
                    </li>
                  ))}
                </ol>
              )}
